...
mads <- apply(data, 2, para_mads)
means <- apply(data, 2, para_means)

data <- data[, means >= quantile(means, para_means) & mads > quantile(mads, para_mads)]

doParallel::registerDoParallel(100)
model_all <- foreach(i = 1:100) %dopar% {
  limit <- 60
  set.seed(i)
  ind <- sample(1:nrow(clin), nrow(clin) * para_sample)
  clin.dis <- clin[ind,]
